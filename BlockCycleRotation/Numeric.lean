...
* The rows of `C` are thinned by the coprimality condition: `∑_{y<x} cTerm`
  averages `0.5966/(ζ(2)x²) ≈ 0.363/x²`, but any *pointwise* row bound must
  cover the primes, where it is `0.5966/x²`.  A row-by-row tail estimate is
...
* For that all-pairs sum, `gTerm(x,y) = 1/(2x(x+y)²) + 1/(2x²(x+y))`, and
  the two pieces are bounded by `1/(4x²)` and `3/(8x²)`: the first telescopes,
  and the second is the reflection `j ↦ x-j`, under which
  `1/(x+j) + 1/(2x-j) ≤ 3x/((x+1)(2x-1))`.  That gives `5/(8x²)` per row,
  within 5% of the truth.

Truncating at `x ≤ 60` (1770 terms) and `ζ(3) ≥ ∑_{d≤50} 1/d³` then closes it.
-/
...
theorem sum_inv_sq_shift_le {x : ℕ} (hx : 0 < x) :
    ∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ)) ^ 2 ≤ 1 / (2 * (x : ℝ)) := by
  have haR : (1 : ℝ) ≤ (x : ℝ) := by exact_mod_cast hx
  have hre : ∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ)) ^ 2
      = ∑ m ∈ Finset.Ioc x (2 * x - 1), 1 / ((m : ℝ)) ^ 2 := by
    refine Finset.sum_bij' (i := fun y _ => x + y) (j := fun m _ => m - x) ?_ ?_ ?_ ?_ ?_
    · intro y h
    ...
    · intro y h
      ...
      ring_nf
  rw [hre]
  have hle : x ≤ 2 * x - 1 := by omega
  have h := sum_inv_sq_Ioc_le hx (2 * x - 1) hle
  have hcast : ((2 * x - 1 : ℕ) : ℝ) = 2 * (x : ℝ) - 1 := by
    have : (1 : ℕ) ≤ 2 * x := by omega
    push_cast [Nat.cast_sub this]
    ring
  rw [hcast] at h
  have hpos : (0 : ℝ) < 2 * (x : ℝ) - 1 := by linarith
  have hstep : 1 / (x : ℝ) - 1 / (2 * (x : ℝ) - 1) ≤ 1 / (2 * (x : ℝ)) := by
...
theorem sum_inv_shift_le (x : ℕ) :
  ...
  · have haR : (2 : ℝ) ≤ (x : ℝ) := by exact_mod_cast hx
    -- the reflected sum
    have hre : ∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ))
        = ∑ y ∈ Finset.Ico 1 x, 1 / (2 * (x : ℝ) - (y : ℝ)) := by
    ...
    have hpair : ∀ j ∈ Finset.Ico 1 x,
        1 / ((x : ℝ) + (j : ℝ)) + 1 / (2 * (x : ℝ) - (j : ℝ))
          ≤ 3 * (x : ℝ) / (((x : ℝ) + 1) * (2 * (x : ℝ) - 1)) := by
      intro j h
      rw [Finset.mem_Ico] at h
      have hj1 : (1 : ℝ) ≤ (j : ℝ) := by exact_mod_cast h.1
      have hja : (j : ℝ) ≤ (x : ℝ) - 1 := by
        have : (j : ℝ) < (x : ℝ) := by exact_mod_cast h.2
        have hji : j + 1 ≤ x := by omega
        have : ((j + 1 : ℕ) : ℝ) ≤ (x : ℝ) := by exact_mod_cast hji
        push_cast at this
        linarith
      have hd1 : (0 : ℝ) < (x : ℝ) + (j : ℝ) := by linarith
      have hd2 : (0 : ℝ) < 2 * (x : ℝ) - (j : ℝ) := by linarith
      have hd3 : (0 : ℝ) < ((x : ℝ) + 1) * (2 * (x : ℝ) - 1) := by nlinarith
      have hprod : ((x : ℝ) + 1) * (2 * (x : ℝ) - 1)
          ≤ ((x : ℝ) + (j : ℝ)) * (2 * (x : ℝ) - (j : ℝ)) := by
      ...
      nlinarith
    have hsum : (∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ)))
        + ∑ y ∈ Finset.Ico 1 x, 1 / (2 * (x : ℝ) - (y : ℝ))
        ≤ ((x : ℕ) - 1 : ℕ) * (3 * (x : ℝ) / (((x : ℝ) + 1) * (2 * (x : ℝ) - 1))) := by
      rw [← Finset.sum_add_distrib]
      calc ∑ y ∈ Finset.Ico 1 x, (1 / ((x : ℝ) + (y : ℝ)) + 1 / (2 * (x : ℝ) - (y : ℝ)))
          ≤ ∑ _y ∈ Finset.Ico 1 x, 3 * (x : ℝ) / (((x : ℝ) + 1) * (2 * (x : ℝ) - 1)) :=
            Finset.sum_le_sum hpair
        _ = ((x - 1 : ℕ) : ℝ) * (3 * (x : ℝ) / (((x : ℝ) + 1) * (2 * (x : ℝ) - 1))) := by
            rw [Finset.sum_const, Nat.card_Ico, nsmul_eq_mul]
    have hcard : ((x - 1 : ℕ) : ℝ) = (x : ℝ) - 1 := by
      push_cast [Nat.cast_sub (by omega : 1 ≤ x)]
      ring
    rw [hcard] at hsum
    have hfin : ((x : ℝ) - 1) * (3 * (x : ℝ) / (((x : ℝ) + 1) * (2 * (x : ℝ) - 1))) ≤ 3 / 2 := by
...
/-- **`∑_{y<x} gTerm ≤ 5/(8x²)`.** -/
theorem gTerm_row_le (x : ℕ) :
    ∑ y ∈ Finset.range x, gTerm (x, y) ≤ 5 / (8 * (x : ℝ) ^ 2) := by
  rcases Nat.eq_zero_or_pos x with rfl | hx
  · simp [gTerm]
  · have haR : (1 : ℝ) ≤ (x : ℝ) := by exact_mod_cast hx
    have hx0 : (0 : ℝ) < (x : ℝ) := by linarith
    have hrange : Finset.range x = insert 0 (Finset.Ico 1 x) := by
      ext k
      rw [Finset.mem_range, Finset.mem_insert, Finset.mem_Ico]
      omega
    have h0 : gTerm (x, 0) = 0 := by simp [gTerm]
    rw [hrange, Finset.sum_insert (by simp), h0, zero_add]
    have hsplit : ∀ y ∈ Finset.Ico 1 x, gTerm (x, y)
        = 1 / (2 * (x : ℝ)) * (1 / ((x : ℝ) + (y : ℝ)) ^ 2)
          + 1 / (2 * (x : ℝ) ^ 2) * (1 / ((x : ℝ) + (y : ℝ))) := by
      intro y h
      rw [Finset.mem_Ico] at h
      unfold gTerm
      rw [if_pos ⟨h.1, h.2⟩]
      have haa : (0 : ℝ) < (x : ℝ) + (y : ℝ) := by positivity
      field_simp
      ring
    rw [Finset.sum_congr rfl hsplit, Finset.sum_add_distrib, ← Finset.mul_sum, ← Finset.mul_sum]
    have hx'1 : 1 / (2 * (x : ℝ)) * (∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ)) ^ 2)
        ≤ 1 / (2 * (x : ℝ)) * (1 / (2 * (x : ℝ))) :=
      mul_le_mul_of_nonneg_left (sum_inv_sq_shift_le hx) (by positivity)
    have hx'2 : 1 / (2 * (x : ℝ) ^ 2) * (∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ)))
        ≤ 1 / (2 * (x : ℝ) ^ 2) * (3 / 4) :=
      mul_le_mul_of_nonneg_left (sum_inv_shift_le x) (by positivity)
    have harith : 1 / (2 * (x : ℝ)) * (1 / (2 * (x : ℝ))) + 1 / (2 * (x : ℝ) ^ 2) * (3 / 4)
        = 5 / (8 * (x : ℝ) ^ 2) := by
      field_simp
      ring
    linarith
...
theorem gTerm_row_support (x y : ℕ) (h : y ∉ Finset.range x) : gTerm (x, y) = 0 := by
...
theorem gTerm_row_summable (x : ℕ) : Summable (fun y => gTerm (x, y)) :=
  summable_of_ne_finset_zero (s := Finset.range x) (gTerm_row_support x)

theorem gTerm_eq_tsum_finRows :
    ∑' p, gTerm p = ∑' x : ℕ, ∑ y ∈ Finset.range x, gTerm (x, y) := by
...
theorem tsum_gTerm_le_partial {N : ℕ} (hN : 0 < N) :
  ...
  refine Real.tsum_le_of_sum_le
    (fun x => Finset.sum_nonneg fun y _ => gTerm_nonneg _) fun s => ?_
  classical
  have hsplit : ∑ x ∈ s, (∑ y ∈ Finset.range x, gTerm (x, y))
      = (∑ x ∈ s.filter (fun x => x ≤ N), ∑ y ∈ Finset.range x, gTerm (x, y))
        + ∑ x ∈ s.filter (fun x => ¬ x ≤ N), ∑ y ∈ Finset.range x, gTerm (x, y) :=
    (Finset.sum_filter_add_sum_filter_not _ _ _).symm
  have hlow : (∑ x ∈ s.filter (fun x => x ≤ N), ∑ y ∈ Finset.range x, gTerm (x, y))
      ≤ ∑ x ∈ Finset.range (N + 1), ∑ y ∈ Finset.range x, gTerm (x, y) := by
    refine Finset.sum_le_sum_of_subset_of_nonneg ?_
      (fun x _ _ => Finset.sum_nonneg fun y _ => gTerm_nonneg _)
    intro x hx
    simp only [Finset.mem_filter, Finset.mem_range] at hx ⊢
    omega
  have hhigh : (∑ x ∈ s.filter (fun x => ¬ x ≤ N), ∑ y ∈ Finset.range x, gTerm (x, y))
      ≤ 5 / (8 * (N : ℝ)) := by
    have h1 : (∑ x ∈ s.filter (fun x => ¬ x ≤ N), ∑ y ∈ Finset.range x, gTerm (x, y))
        ≤ ∑ x ∈ s.filter (fun x => ¬ x ≤ N), 5 / (8 * (x : ℝ) ^ 2) :=
      Finset.sum_le_sum fun x _ => gTerm_row_le x
    have h2 : (∑ x ∈ s.filter (fun x => ¬ x ≤ N), 5 / (8 * (x : ℝ) ^ 2))
        = (5 / 8) * ∑ x ∈ s.filter (fun x => ¬ x ≤ N), 1 / ((x : ℝ) ^ 2) := by
      rw [Finset.mul_sum]
...
theorem gTerm_partial_ge :
    (2443 : ℝ) / 10000 ≤ ∑ x ∈ Finset.range 61, ∑ y ∈ Finset.range x, gTerm (x, y) := by
  norm_num [gTerm, Finset.sum_range_succ]
...
theorem cConst_ge : (2025 : ℝ) / 10000 ≤ cConst := by
  ...
  have hrowsum : Summable (fun x : ℕ => ∑ y ∈ Finset.range x, gTerm (x, y)) := by
    refine hrows.2.congr fun x => ?_
    exact tsum_eq_sum (gTerm_row_support x)
  have hpartial : (∑ x ∈ Finset.range 61, ∑ y ∈ Finset.range x, gTerm (x, y))
      ≤ ∑' p, gTerm p := by
    rw [gTerm_eq_tsum_finRows]
    exact hrowsum.sum_le_tsum _ (fun x _ => Finset.sum_nonneg fun y _ => gTerm_nonneg _)
  have hG : zeta3 * cConst = ∑' p, gTerm p := by
    rw [zeta3_mul_cConst, tsum_gTerm]
...
bound from below.  The reflection gives `∑_{y<x} gTerm ≥ 5(x-1)/(9x³)`, which
for `x ≥ 61` is at least `0.54/x²`, and `∑_{x>60} 1/x² ≥ 1/61` telescopes. -/

theorem gTerm_row_ge {x : ℕ} (hx : 61 ≤ x) :
    (54 / 100 : ℝ) / (x : ℝ) ^ 2 ≤ ∑ y ∈ Finset.range x, gTerm (x, y) := by
  have hx1 : 1 ≤ x := by omega
  have haR : (61 : ℝ) ≤ (x : ℝ) := by exact_mod_cast hx
  have hx0 : (0 : ℝ) < (x : ℝ) := by linarith
  have hrange : Finset.range x = insert 0 (Finset.Ico 1 x) := by
    ext k
    rw [Finset.mem_range, Finset.mem_insert, Finset.mem_Ico]
  ...
  have hsplit : ∀ y ∈ Finset.Ico 1 x, gTerm (x, y)
    ...
    ring
  rw [Finset.sum_congr rfl hsplit, Finset.sum_add_distrib, ← Finset.mul_sum, ← Finset.mul_sum]
  have hx'1 : 1 / (2 * (x : ℝ)) * (4 * ((x : ℝ) - 1) / (9 * (x : ℝ) ^ 2))
      ≤ 1 / (2 * (x : ℝ)) * (∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ)) ^ 2) :=
    mul_le_mul_of_nonneg_left (sum_inv_sq_shift_ge hx1) (by positivity)
  have hx'2 : 1 / (2 * (x : ℝ) ^ 2) * (2 * ((x : ℝ) - 1) / (3 * (x : ℝ)))
      ≤ 1 / (2 * (x : ℝ) ^ 2) * (∑ y ∈ Finset.Ico 1 x, 1 / ((x : ℝ) + (y : ℝ))) :=
    mul_le_mul_of_nonneg_left (sum_inv_shift_ge hx1) (by positivity)
  have harith : (54 / 100 : ℝ) / (x : ℝ) ^ 2
      ≤ 1 / (2 * (x : ℝ)) * (4 * ((x : ℝ) - 1) / (9 * (x : ℝ) ^ 2))
        + 1 / (2 * (x : ℝ) ^ 2) * (2 * ((x : ℝ) - 1) / (3 * (x : ℝ))) := by
    rw [div_le_iff₀ (by positivity)]
    have hexp : (1 / (2 * (x : ℝ)) * (4 * ((x : ℝ) - 1) / (9 * (x : ℝ) ^ 2))
        + 1 / (2 * (x : ℝ) ^ 2) * (2 * ((x : ℝ) - 1) / (3 * (x : ℝ)))) * (x : ℝ) ^ 2
        = 5 * ((x : ℝ) - 1) / (9 * (x : ℝ)) := by
      field_simp
      ring
...
theorem tsum_gTerm_ge :
    (∑ x ∈ Finset.range 61, ∑ y ∈ Finset.range x, gTerm (x, y)) + (54 / 100) * (1 / 61)
      ≤ ∑' p, gTerm p := by
  have hnn : (0 : ℕ × ℕ → ℝ) ≤ gTerm := fun p => gTerm_nonneg p
  have hrows := (summable_prod_of_nonneg hnn).1 gTerm_summable
  have hrowsum : Summable (fun x : ℕ => ∑ y ∈ Finset.range x, gTerm (x, y)) := by
    refine hrows.2.congr fun x => ?_
    exact tsum_eq_sum (gTerm_row_support x)
  have hshift : Summable (fun j : ℕ => ∑ y ∈ Finset.range (j + 61), gTerm (j + 61, y)) :=
    (summable_nat_add_iff (f := fun x : ℕ => ∑ y ∈ Finset.range x, gTerm (x, y)) 61).2 hrowsum
  have hgs : Summable (fun j : ℕ => (54 / 100 : ℝ) * (1 / ((j : ℝ) + 61) ^ 2)) := by
    ...
    have h1 : Summable (fun j : ℕ => 1 / ((j : ℝ) + 61) ^ 2) := by
      ...
      ring_nf
    exact h1.mul_left _
  have hsplit : (∑ x ∈ Finset.range 61, ∑ y ∈ Finset.range x, gTerm (x, y))
      + ∑' j : ℕ, (∑ y ∈ Finset.range (j + 61), gTerm (j + 61, y))
      = ∑' p, gTerm p := by
    rw [gTerm_eq_tsum_finRows]
    exact hrowsum.sum_add_tsum_nat_add 61
  have htail : (54 / 100 : ℝ) * (1 / 61)
      ≤ ∑' j : ℕ, (∑ y ∈ Finset.range (j + 61), gTerm (j + 61, y)) := by
    have hterm : ∀ j : ℕ, (54 / 100 : ℝ) * (1 / ((j : ℝ) + 61) ^ 2)
        ≤ ∑ y ∈ Finset.range (j + 61), gTerm (j + 61, y) := by
      intro j
      have h := gTerm_row_ge (x := j + 61) (by omega)
