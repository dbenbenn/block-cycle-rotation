...
| `log x` factor, `∑ 1/min(b,x−b)` | Lemma 16 | `sum_inv_min_le` | ✅ proved |
| Character orthogonality | §4 | `sum_e_root` | ✅ proved |
| Inner sum `∑(A+B·y')·e(2πb·y'/x)` | §4 | `norm_linear_geom_sum_root_le` | ✅ proved |
...
| Same, explicit `log x` | §4 | `sum_ap_sub_main_le_log` | ✅ proved |
| Continuant `K` | Heilbronn 1969 | `BlockCycleRotation.K` | ✅ defined |
| Euler's continuant identity | (gives `n = x·x' + y·y'`) | `K_append` | ✅ proved |
| Continuants are palindromic | Heilbronn 1969 | `K_reverse` | ✅ proved |
| Consecutive continuants coprime | (gives `gcd(x,y) = 1`) | `K_coprime` | ✅ proved |
| Size conditions `x > y ≥ 1` | Heilbronn 1969 | `K_dropLast_lt'`, `K_tail_lt'` | ✅ proved |
...
| Quadruples symmetric, `∑x = ∑x'` | §4 | `sum_fst_eq_sum_snd` | ✅ proved |
| Shifts classified by `gcd(n,k)` | §4 | `sum_allShifts_eq` | ✅ proved |
| Quadruples classified by `gcd(x',y')` | §4 | `sum_snd_quadruplesAll` | ✅ proved |
| **Equation (eq. heilbron)** | §4 | `heilbron` | ✅ **proved** |
| Triple sum: inner sum over `y'` | §4 | `inner_sum_sub_main_le` | ✅ proved |
...
| `x'`-elimination to triples | §4 | `sum_snd_quadruplesQ_eq_triples` | ✅ proved |
| Coprime `x'`-elimination | §4 | `sum_snd_quadruplesAll_eq` | ✅ proved |
...
| Symmetrisation `x' > x` | §4 | `Q_symmetrise` | ✅ proved |
| Diagonal is `O(n^{1+ε})` | §4 | `sum_diag_isBigO` | ✅ proved |
| `x' > x` through the classification | §4 | `sum_QGT_classify` | ✅ proved |
| **The paper's triple sum** | §4 | `Q_gt_tripleSum` | ✅ **proved** |
| Key restriction `d·x² < m` | §4 | `gtTriples_sq_lt` | ✅ proved |
...
| Admissible `x` count `≤ √((m−1)/d)` | §4 | `card_a_le` | ✅ proved |
...
| Tail bound `∑_{x>N} 1/x² ≤ 1/N` | Lemma 19 | `sum_inv_sq_tail_le` | ✅ proved |
...
| Small branch: `2d·x² > m` | Lemma 19 | `small_two_mul_gt` | ✅ proved |
| AP counting `≤ Y/x + 1` | Lemma 19 | `card_mod_filter_le` | ✅ proved |
...
| Sharper `cTerm ≤ 1/x³`, tail `≤ 1/N` | — new | `cTerm_le_cube`, `cTerm_row_le_sq`, `cConst_le_partial_add_sharp` | ✅ proved |
| Certified `partial(20) ≤ 39/200` | — new | `cConst_partial_le` | ✅ proved |
| **`D < 2`** (beats trinity rotation on average) | Remark 15, asserted | `dConst_lt_two` | ✅ **proved** |
| `1 < D` | — new | `one_lt_dConst` | ✅ proved |
| Row bound `∑_{y<x} gTerm ≤ 5/(8x²)` | — new | `gTerm_row_le` | ✅ proved |
| `ζ(3) ∈ [1.2018, 1.2023]` | — new | `zeta3_ge`, `zeta3_le` | ✅ proved |
| **`D ≤ 1.85`**, the paper's value | Thm 14, numerically | `dConst_le_185` | ✅ **proved** |
| Row bound from below `≥ 5(x−1)/(9x³)` | — new | `gTerm_row_ge` | ✅ proved |
| **`1.84 ≤ D ≤ 1.85`** | — new (paper gives `≈ 1.85`) | `dConst_enclosure` | ✅ **proved** |
...
condition `y' ≡ a mod x` by additive characters: at a nontrivial `x`-th root of
unity the chord length satisfies `‖e(2πb/x) − 1‖ ≥ 4·min(b, x−b)/x`, so

```
‖∑_{B ≤ j < T} e(2πmj/a)‖ ≤ a / (2·min(m, a−m)).
```

Summing `1/min(b, x−b)` over `0 < b < x` is what produces the `log x` in Lemma 16;
...
The sums it must control are `∑_{1≤y'<Y, y'≡a mod x} (A + B·y')`; expanding the
congruence in characters turns the error into a weighted sum of
`∑_{y'} (A + B·y')·e(2πb·y'/x)` over nontrivial `m`, with phases of modulus one, and

```
‖∑_m w_m ∑_b (A + B·b) e(2πmb/a)‖ ≤ (‖A‖ + ‖B‖(T−1)) · a · ∑_{0<m<a} 1/m,
```

whose harmonic sum is the `log x` of Lemma 16. `Progression.lean` then
...
| `b`, `e_x` | `b`, `e (2*pi*b/x)` | character index; the paper's `e_x` carries the `2*pi`, `ExpSum.e` does not |
...
**The certified numerics are new.** The paper states `D ≈ 1.85`, says the
constant is "explicitly known" as `D = 1 + 4C` with `C` given by two quickly
convergent double series, and describes it as "estimated numerically"; Remark 15
asserts `D < 2` on that basis. It proves no bound on either. Everything in
`Bound.lean` and `Numeric.lean` — the sharper termwise bound `cTerm ≤ 1/x³`, the
row bounds, the tail estimates, the truncations at `N = 20` and `N = 60`, the
enclosure `ζ(3) ∈ [1.2018, 1.2023]`, and the resulting `1.84 ≤ D ≤ 1.85` and
`D < 2` — is supplied here to turn those numerical estimates into proofs. Rows
